    ...
    <div className="w-full max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {filteredTournaments.map((t) => (
          <div key={t.id} className="group flex flex-col bg-zinc-900/30 border border-white/10 hover:border-white/30 transition-all duration-500">
            ...
            <div className="h-64 w-full relative overflow-hidden">
                ...
                <div className="absolute top-4 right-4">
                    {t.status === 'upcoming' ? (
                        <span className="bg-white text-black text-[10px] font-bold px-3 py-1 uppercase tracking-widest">Upcoming</span>
                    ) : (
                        <span className="bg-zinc-800 text-zinc-400 border border-white/10 text-[10px] font-bold px-3 py-1 uppercase tracking-widest">Completed</span>
                    )}
                </div>
            </div>
            ...
            <div className="p-8 flex flex-col flex-1 relative">
                ...
                <div className="mb-6">
                    <span className="text-[10px] text-zinc-500 uppercase tracking-[0.2em] block mb-2">{t.league}</span>
                    <h3 className="text-2xl font-bold text-white mb-3 tracking-tight group-hover:text-zinc-200 transition-colors">{t.name}</h3>
                    ...
                    <div>
                        {t.result ? (
                             <div className="flex flex-col gap-1">
                                <span className="text-[10px] uppercase text-zinc-600 tracking-widest">Result</span>
                                <div className="flex items-center gap-2 text-white">
                                    <Trophy size={16} className="text-zinc-400" />
                                    <span className="font-medium tracking-wider text-lg">{t.result}</span>
                                </div>
                             </div>
                        ) : (
                             <div className="flex flex-col gap-1">
                                <span className="text-[10px] uppercase text-zinc-600 tracking-widest">Prize Pool</span>
                                <div className="flex items-center gap-1 text-white">
                                    <span className="font-medium tracking-wider text-lg">{t.prize}</span>
                                </div>
                             </div>
                        )}
                    </div>
