...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-obsidian flex flex-col">
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-obsidian/90 backdrop-blur-md border-b border-white/5 py-4' : 'bg-transparent py-8'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="text-2xl font-bold tracking-[0.3em] text-white">COLOSSUS</div>
          
          {/* Desktop Menu */}
          <div className="hidden md:flex gap-12 text-xs tracking-[0.2em] font-light text-zinc-400">
            <button onClick={() => navigateTo('home', 'roster')} className="hover:text-white transition-colors">ROSTER</button>
            <button onClick={() => navigateTo('home', 'matches')} className="hover:text-white transition-colors">MATCHES</button>
            <button onClick={() => navigateTo('home', 'tournaments')} className="hover:text-white transition-colors">TOURNAMENTS</button>
            <button onClick={() => navigateTo('config')} className={`transition-colors ${currentPage === 'config' ? 'text-white' : 'hover:text-white'}`}>CONFIG</button>
            <button onClick={() => navigateTo('home', 'contact')} className="hover:text-white transition-colors">CONTACT</button>
          </div>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-black flex items-center justify-center">
          <div className="flex flex-col gap-8 text-center text-2xl font-light tracking-widest text-zinc-400">
            ...
            <button onClick={() => navigateTo('home', 'matches')}>MATCHES</button>
            <button onClick={() => navigateTo('home', 'tournaments')}>TOURNAMENTS</button>
            <button onClick={() => navigateTo('config')}>CONFIG</button>
            <button onClick={() => navigateTo('home', 'contact')}>CONTACT</button>
          </div>
        </div>
      )}
            {/* Main Content */}
      ...
      <section className="relative h-screen flex flex-col justify-center items-center overflow-hidden">
        ...
        <div className="z-10 text-center px-4 w-3xl">
          ...
          <p className="text-zinc-400 max-w-md mx-auto text-sm md:text-base font-light leading-relaxed mb-12">
            Latin American e-sports team competing at the highest level in Left 4 Dead 2 tournaments worldwide.
          </p>
          <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
            <button 
             ...
             className="group relative px-8 py-3 bg-white text-black text-sm tracking-widest font-medium overflow-hidden">
              <span className="relative z-10 group-hover:text-white transition-colors duration-300">EXPLORE ROSTER</span>
            ...
            <button
              onClick={() => window.open("https://steamcommunity.com/groups/colossusl4d2", "_blank")} 
      ...
      {/* Matches Section */}
      <section id="matches" className="py-32 relative border-y border-white/5 bg-zinc-950/50">
        <div className="max-w-7xl mx-auto px-6">
           <div className="flex flex-col items-center mb-16">
             <h2 className="text-3xl font-light tracking-tight text-white text-center">COMPETITIVE FEED</h2>
      ...
      <section id="roster" className="py-32 max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20">
           <div>
              ...
              <h2 className="text-3xl font-light tracking-tight relative z-10">Active Roster</h2>
           </div>
           <div className="h-px flex-1 bg-linear-to-r from-zinc-800 to-transparent mx-8 mb-2 hidden md:block" />
           <div className="text-zinc-500 text-sm tracking-widest">SEASON 2025</div>
        </div>
      ...
      <section id="tournaments" className="py-32 bg-zinc-900/30 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6">
             <div className="flex flex-col items-center mb-16">
                 <h2 className="text-3xl font-light tracking-tight text-white text-center">MAJOR TOURNAMENTS</h2>
                 <div className="w-12 h-px bg-white/20 mt-4"></div>
             </div>
             <TournamentsList />
        </div>
      ...
      <footer className="py-16 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex gap-6 text-zinc-500">
             <a href="https://www.youtube.com/@ColossusPOV" target="_blank" className="hover:text-white transition-colors text-xs tracking-widest uppercase">Youtube</a>
             <a href="https://steamcommunity.com/groups/colossusl4d2" target="_blank" className="hover:text-white transition-colors text-xs tracking-widest uppercase">Steam</a>
             <a href="#" className="hover:text-white transition-colors text-xs tracking-widest uppercase">Discord</a>
          </div>
        </div>
      </footer>
